...
'''
##add in try fseq - if fail then report check fseq
#change bamToBed to bedtools bamtobed
...
def readLines(file, window):
    ...
    for f in range(0,end):
        line = file.readline()
        if line == '': break #Break if line is empty, end of file reached
        columns = line.split("\t")
...
def readLinesSize(file, size):
    """
    Reads in a wig file up to a customized point depending on the respective wig files. Will read in up to a specified bp position 
...
def findOp(ctrlList, expmList, zzcut, outName, distance, wind, shif):   
    ...
    print "Length before slice"
    for i in range(len(readCtrlArray)):
	print len(readCtrlArray[i]), ":", len(readExpmArray[i])
    ...
    start = int(maxVal)
    print start, ":"
    for i in range(len(startCtrlList)):
	print  startCtrlList[i], startExpmList[i]
	...
	#Begin looping through the files one window at a time 
		...
		for k in range(len(readCtrlArray)):
			...
			if tempCVal == 0: tempCVal = 0.0001 #0.0001
			if tempEVal == 0: tempEVal = 0.0001
			totalCtrl[k].append(tempCVal)
			totalExpm[k].append(tempEVal)	
			#totValCtrl[k] = totValCtrl[k] + tempCVal
			#totValExpm[k] = totValExpm[k] + tempEVal
	    ...
	    else: #if there are not at least 3 replicates, can't do t-test between means, do t-test versus each window of each replicate. 
     	    	statArray = []
            	for k in range(len(totalCtrl)):
	    ...
	    if (zVal > zCut): 
		flag2 = True
		#Require all ctrl file mean values to be above expm mean values by a certain amount
...
def vararg_callback(option, opt_str, value, parser):
    ...
    for arg in parser.rargs:
        # stop on --check like options
        if arg[:2] == "--" and len(arg) > 2:
            break
        # stop on -d, but not on -6 or -5.0
...
if __name__ == '__main__':
    ...
    #Add flag options
    usage = "If BAM files run \n usage: %prog -c [ctrl1.bam] [ctrl2.bam] [ctrl3.bam] -e [expm1.bam] [expm2.bam] [expm3.bam] -z [zcut] -o [output] -n [chromosomes] \n This will convert BAM files to BED file and run FSEQ on each BED file to report WIG files. Once that finishes run the next command on each chromosome wig file \n usage: %prog -c [ctrl1_1.wig] [ctrl2_1.wig] [ctrl3_1.wig] -e [expm1_1.wig] [expm2_1.wig] [expm3_1.wig] -z [zcut] -o [output] -n [chromosomes] -w"
    ...
    parser.add_option('-w', '--wig',  action="store_true", dest="wig",
	    help='If you have already run F-Seq or have wig files and wish to head directly to finding open sites. Each chromosome must be run separately in this mode')
    parser.add_option('-n', '--chr', dest='chr', type = "int",
	    help='Number of chromosomes your species contains')
    ...
    if not (options.chr):
	parser.error("Must provide the number of chromosomes")
    if not(options.wig): #If the wig option is not selected, then bam files are given. Run commands to convert to Bed, run Fseq to optain wig files, and then find open regions
	...
	try:
		...
		print "F-Seq is not downloaded or available in path"
	...
	print("Converting control bam files to bed files.")
		...
		cmd = 'bamToBed -cigar -i {} > {}.bed'.format(f, fileTag)
		print(cmd)
	...
	for f in fileC:
		os.chdir(f)
		for g in [x+1 for x in range(options.chr)]:
			cmd = 'cp {}.wig ../run/{}_{}.wig'.format(g, f, g)
			os.system(cmd)
		os.chdir("..")


	#go through experimental files
